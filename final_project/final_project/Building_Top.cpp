...
Building_Top::Building_Top() {
    ...
    //    int width = 750;
    //837x750
    GLubyte *data = ppmRead("../../../../../../images/building/ppm/top.PPM", &imgsize, &imgsize);
...
//----------------------------------------------------------------------------
void Building_Top::makeQuad(int ind1, int ind2, int ind3, int ind4) {
...
void Building_Top::makeIndexedQuad(GLubyte ind1, GLubyte ind2, GLubyte ind3, GLubyte ind4) {
...
//set the materials
void Building_Top::setMaterial(vec4 d, vec4 a, vec4 s, float sh) {
    matDiffuse = d;
    matAmbient = a;
    matSpecular = s;
    shininess = sh;
...
void Building_Top::draw(Camera cam, vector<Light> lights){
...
void Building_Top::testCollision(vec4 rayOrigin, vec4 rayDirection)
{
